...
const FILE_URL = 'https://media.base44.com/files/public/69834d9e0d7220d671bfd124/f49370f15_NFMT26.xlsx';
const FILE_NAME = 'NFMT26.xlsx';
const DEFAULT_SHOW_NAME = 'NFMT 26';
const DEFAULT_DEALER_INSTANCE_ID = '69d94c853e7e9e5c36953834';

function splitName(fullName) { const normalized = String(fullName || '').trim(); if (!normalized) return { first_name: '', last_name: '' }; const parts = normalized.split(/\s+/); return { first_name: parts[0] || '', last_name: parts.slice(1).join(' ') }; }
function normalizePhone(value) { if (value === null || value === undefined || value === '') return ''; return String(value).replace(/\.0$/, '').trim(); }
function normalizeWebsite(value) { const normalized = String(value || '').trim(); if (!normalized) return ''; if (normalized.startsWith('http://') || normalized.startsWith('https://')) return normalized; return `https://${normalized}`; }
function pick(record, key) { return record?.[key] ?? record?.data?.[key] ?? null; }
function companyKey(companyName, website) { return `${String(companyName || '').trim().toLowerCase()}::${String(website || '').trim().toLowerCase()}`; }
...
Deno.serve(async (req) => {
  try {
    ...
    if (!response.ok) return Response.json({ error: 'Failed to download Excel file' }, { status: 500 });
    ...
    for (let index = 0; index < rows.length; index += 1) {
      ...
      if (existingLead) { skipped += 1; continue; }

      let account = accountMap.get(companyKey(companyName, website)) || accountMap.get(companyKey(companyName, ''));
      if (!account && companyName) {
        account = await base44.asServiceRole.entities.Account.create({ name: companyName, company_name: companyName, website, phone: phone || tollFree, dealer_instance_id: DEFAULT_DEALER_INSTANCE_ID, owner_user_id: user.id });
        accountMap.set(companyKey(companyName, website), account);
        accountMap.set(companyKey(companyName, ''), account);
        createdAccounts += 1;
      }

      leadsToCreate.push({ first_name, last_name, full_name: contactName || companyName, email, phone, title: contactType, department: '', company_name: companyName, website, toll_free: tollFree, contact_type: contactType, show_name: DEFAULT_SHOW_NAME, source_file_name: FILE_NAME, source_row_index: index + 1, account_id: account?.id || null, dealer_instance_id: DEFAULT_DEALER_INSTANCE_ID, owner_user_id: user.id, status: 'open' });
    }

    if (leadsToCreate.length > 0) await base44.asServiceRole.entities.Lead.bulkCreate(leadsToCreate);
