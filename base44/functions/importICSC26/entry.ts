...
const FILE_URL = 'https://media.base44.com/files/public/69834d9e0d7220d671bfd124/4424efc96_ICSC26.xlsx';
const FILE_NAME = 'ICSC26.xlsx';
const DEFAULT_SHOW_NAME = 'ICSC 26';
  ...
  try {
    ...
    for (let index = 0; index < rows.length; index += 1) {
      ...
      if (!account && companyName) {
        account = await base44.asServiceRole.entities.Account.create({ name: companyName, company_name: companyName, website, phone, dealer_instance_id: DEFAULT_DEALER_INSTANCE_ID, owner_user_id: user.id });
        accountMap.set(companyKey(companyName, website), account);
        accountMap.set(companyKey(companyName, ''), account);
        createdAccounts += 1;
      }

      leadsToCreate.push({ first_name, last_name, full_name: contactName || companyName, email, phone, title: contactType, department: '', company_name: companyName, website, contact_type: contactType, show_name: DEFAULT_SHOW_NAME, source_file_name: FILE_NAME, source_row_index: index + 1, account_id: account?.id || null, dealer_instance_id: DEFAULT_DEALER_INSTANCE_ID, owner_user_id: user.id, status: 'open' });
    }
