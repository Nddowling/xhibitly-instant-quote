...
export default function ProductCard({ product }) {
  ...
  return (
    <motion.div
      ...
      className="group bg-white/4 hover:bg-white/6 border border-white/8 hover:border-[#e2231a]/30 rounded-2xl overflow-hidden cursor-pointer transition-all"
    >
      {/* Image */}
      <div className="aspect-square bg-white/5 overflow-hidden relative">
        ...
        {product.sku && (
          <Badge className="absolute top-3 left-3 bg-[#e2231a] text-white text-xs font-bold border-0">
            {product.sku}
          </Badge>
        )}
      </div>
      ...
      {/* Info */}
      <div className="p-4">
        <p className="text-xs text-white/35 mb-1.5">{product.category}</p>
        ...
        <div className="flex items-center gap-1 mb-3">
          {[1,2,3,4,5].map(i => <Star key={i} className="w-3 h-3 fill-[#e2231a] text-[#e2231a]" />)}
        ...
        <div className="flex items-center justify-between gap-2">
          ...
          <Button
            ...
            className="bg-[#e2231a] hover:bg-[#c91e16] text-white font-semibold h-8 px-3 text-xs rounded-lg flex-shrink-0"
          >
