...
export default function MessageBubble({ message, onAddProduct }) {
    ...
    return (
        <div className={cn("flex gap-3", isUser ? "justify-end" : "justify-start")}>
            {!isUser && (
                <div className="h-7 w-7 rounded-lg bg-[#0D4FB3]/10 flex items-center justify-center mt-0.5 shrink-0">
                    <div className="h-1.5 w-1.5 rounded-full bg-[#0D4FB3]" />
                </div>
            )}
            <div className={cn("max-w-[85%]", isUser && "flex flex-col items-end")}>
                {message.content && (
                    <div className={cn(
                        ...
                        isUser ? "bg-[#0D4FB3] text-white" : "bg-white border border-[#0D4FB3]/20 text-slate-900"
                    )}>
                        {isUser ? (
