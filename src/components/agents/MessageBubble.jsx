        ...
        <div className="mt-2 text-xs">
            <button
                ...
                className={cn(
                    ...
                    expanded ? "bg-slate-50 dark:bg-slate-800 border-slate-300 dark:border-slate-700" : "bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800"
                )}
            >
                <Icon className={cn("h-3 w-3", statusConfig.color, statusConfig.spin && "animate-spin")} />
                <span className="text-slate-700 dark:text-slate-300">{formattedName}</span>
                {statusConfig.text && (
                    <span className={cn("text-slate-500 dark:text-slate-400", isError && "text-red-600 dark:text-red-500")}>
                        • {statusConfig.text}
                    </span>
                )}
...
export default function MessageBubble({ message }) {
    ...
    return (
        <div className={cn("flex gap-3", isUser ? "justify-end" : "justify-start")}>
            {!isUser && (
                <div className="h-7 w-7 rounded-lg bg-slate-100 dark:bg-slate-800 flex items-center justify-center mt-0.5 shrink-0">
            ...
            <div className={cn("max-w-[85%]", isUser && "flex flex-col items-end")}>
                {message.content && (
                    <div className={cn(
                        ...
                        isUser ? "bg-primary text-primary-foreground" : "bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-900 dark:text-white"
                    )}>
                        {isUser ? (
                            ...
                            <ReactMarkdown 
                                ...
                                components={{
                                    ...
                                    blockquote: ({ children }) => (
                                        <blockquote className="border-l-2 border-slate-300 dark:border-slate-700 pl-3 my-2 text-slate-600 dark:text-slate-400">
                                            {children}
                                        </blockquote>
                                    ),
                                }}
