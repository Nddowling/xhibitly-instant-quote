...
function BoothModel({ modelPath, brandColors, boothSize }) {
  ...
  let gltf;
  try {
    // Only try to load if the model path looks valid and we haven't had an error
    if (!loadError && modelPath && !modelPath.includes('booth-sample-1.glb')) {
      gltf = useGLTF(modelPath, true);
    } else {
      // Use fallback for the large sample file or if there's an error
      if (!useFallback) setUseFallback(true);
    }
  } catch (error) {
    console.warn('GLB model not available, using fallback geometry:', error);
    if (!useFallback) {
      setUseFallback(true);
      setLoadError(true);
    }
...
// Don't preload large GLB models - they will load on demand
